...
def symbolic_tseries(input_size, order):
    x_syms = sp.symbols('x:' + str(int(input_size / 2)))
    xdot_syms = sp.symbols('xdot:' + str(int(input_size / 2)))
    ...
    h: sp.Expr = -g.subs(y, Bx + d) - dg.subs(y, Bx + d) * Bxdot # -g(Bx + d) - dg(Bx + d) * Bxdot    
